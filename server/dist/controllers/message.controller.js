"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
const repository_1 = require("@loopback/repository");
const rest_1 = require("@loopback/rest");
const models_1 = require("../models");
const repositories_1 = require("../repositories");
let MessageController = class MessageController {
    constructor(messageRepository) {
        this.messageRepository = messageRepository;
    }
    async create(message) {
        return await this.messageRepository.create(message);
    }
    async count(where) {
        return await this.messageRepository.count(where);
    }
    async find(filter) {
        return await this.messageRepository.find(filter);
    }
    async updateAll(message, where) {
        return await this.messageRepository.updateAll(message, where);
    }
    async findById(id) {
        return await this.messageRepository.findById(id);
    }
    async updateById(id, message) {
        await this.messageRepository.updateById(id, message);
    }
    async replaceById(id, message) {
        await this.messageRepository.replaceById(id, message);
    }
    async deleteById(id) {
        await this.messageRepository.deleteById(id);
    }
};
__decorate([
    rest_1.post('/messages', {
        responses: {
            '200': {
                description: 'Message model instance',
                content: { 'application/json': { schema: { 'x-ts-type': models_1.Message } } },
            },
        },
    }),
    __param(0, rest_1.requestBody()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [models_1.Message]),
    __metadata("design:returntype", Promise)
], MessageController.prototype, "create", null);
__decorate([
    rest_1.get('/messages/count', {
        responses: {
            '200': {
                description: 'Message model count',
                content: { 'application/json': { schema: repository_1.CountSchema } },
            },
        },
    }),
    __param(0, rest_1.param.query.object('where', rest_1.getWhereSchemaFor(models_1.Message))),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], MessageController.prototype, "count", null);
__decorate([
    rest_1.get('/messages', {
        responses: {
            '200': {
                description: 'Array of Message model instances',
                content: {
                    'application/json': {
                        schema: { type: 'array', items: { 'x-ts-type': models_1.Message } },
                    },
                },
            },
        },
    }),
    __param(0, rest_1.param.query.object('filter', rest_1.getFilterSchemaFor(models_1.Message))),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], MessageController.prototype, "find", null);
__decorate([
    rest_1.patch('/messages', {
        responses: {
            '200': {
                description: 'Message PATCH success count',
                content: { 'application/json': { schema: repository_1.CountSchema } },
            },
        },
    }),
    __param(0, rest_1.requestBody()),
    __param(1, rest_1.param.query.object('where', rest_1.getWhereSchemaFor(models_1.Message))),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [models_1.Message, Object]),
    __metadata("design:returntype", Promise)
], MessageController.prototype, "updateAll", null);
__decorate([
    rest_1.get('/messages/{id}', {
        responses: {
            '200': {
                description: 'Message model instance',
                content: { 'application/json': { schema: { 'x-ts-type': models_1.Message } } },
            },
        },
    }),
    __param(0, rest_1.param.path.number('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", Promise)
], MessageController.prototype, "findById", null);
__decorate([
    rest_1.patch('/messages/{id}', {
        responses: {
            '204': {
                description: 'Message PATCH success',
            },
        },
    }),
    __param(0, rest_1.param.path.number('id')),
    __param(1, rest_1.requestBody()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, models_1.Message]),
    __metadata("design:returntype", Promise)
], MessageController.prototype, "updateById", null);
__decorate([
    rest_1.put('/messages/{id}', {
        responses: {
            '204': {
                description: 'Message PUT success',
            },
        },
    }),
    __param(0, rest_1.param.path.number('id')),
    __param(1, rest_1.requestBody()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, models_1.Message]),
    __metadata("design:returntype", Promise)
], MessageController.prototype, "replaceById", null);
__decorate([
    rest_1.del('/messages/{id}', {
        responses: {
            '204': {
                description: 'Message DELETE success',
            },
        },
    }),
    __param(0, rest_1.param.path.number('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", Promise)
], MessageController.prototype, "deleteById", null);
MessageController = __decorate([
    __param(0, repository_1.repository(repositories_1.MessageRepository)),
    __metadata("design:paramtypes", [repositories_1.MessageRepository])
], MessageController);
exports.MessageController = MessageController;
//# sourceMappingURL=message.controller.js.map